...
Several metrics in this harness use an LLM (GPT-4o via OpenAI) to evaluate the quality of another LLM's output. This is called LLM-as-judge.
...
| Answer Relevancy | Embedding-based | Whether the response stays focused on the bug in question and does not include off-topic content |
...
| Answer Relevancy | Embedding-based | Whether the explanation addresses what the code does, not tangential topics |
...
| Answer Relevancy | Embedding-based | Whether the generated answer addresses the actual question asked |
...
| AnswerRelevancyMetric | 0.0 – 1.0 | Embedding similarity between response and question |
...
- **AnswerRelevancyMetric** — monitor scores in Step 4 runs. Embedding similarity tends to score high by default. If the metric never fails anything useful, introduce a separate `ANSWER_RELEVANCY_THRESHOLD` and tighten it.
